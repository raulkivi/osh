...
class TestParseQaVerdicts:
    def test_parses_valid_lines(self):
        ...
        assert nlsh.parse_qa_verdicts(response) == [
            ("PASS", "Looks safe"),
            ("FAIL", "Deletes data"),
            ("WARN", "Irreversible"),
        ]
    ...
    def test_ignores_out_of_range_numbers(self):
        assert nlsh.parse_qa_verdicts("7|PASS|out of range") == []
        assert nlsh.parse_qa_verdicts("0|PASS|out of range") == []

    def test_ignores_invalid_verdict_words(self):
        assert nlsh.parse_qa_verdicts("1|MAYBE|not a real verdict") == []

    def test_ignores_malformed_lines(self):
        assert nlsh.parse_qa_verdicts("not a verdict line at all") == []

    def test_reason_is_optional(self):
        assert nlsh.parse_qa_verdicts("1|PASS") == [("PASS", "")]

    def test_blank_input_returns_empty_list(self):
        assert nlsh.parse_qa_verdicts("") == []
